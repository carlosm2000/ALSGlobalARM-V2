...
Function Get-SoftwareJSON{
  if(Test-Path -Path .\AdditionalFiles\SelfHostedAgent\software.json) {
    $paramDictionary = New-Object System.Management.Automation.RuntimeDefinedParameterDictionary

    Get-Content -Path .\AdditionalFiles\SelfHostedAgent\software.json | ConvertFrom-Json | Get-Member | Where-Object -FilterScript {$_.MemberType -eq "NoteProperty" -and $_.Name -ne "VSTS"} | Select-Object -ExpandProperty Name | ForEach-Object -Process {
  ...
  Function Install-Component{
    ...
    if($Component.AdditionalCmd) {
      Write-Host "Additional command Found"
      Write-Host $Component.AdditionalCmd
      $env:Path = [System.Environment]::GetEnvironmentVariable("Path","Machine") + ";" + [System.Environment]::GetEnvironmentVariable("Path","User") 
      Invoke-Expression $Component.AdditionalCmd
    }
  }
  ...
  if(Test-Path -Path .\AdditionalFiles\SelfHostedAgent\software.json) {
    $Software = Get-Content -Path .\AdditionalFiles\SelfHostedAgent\software.json | ConvertFrom-Json
  }
  else {
  ...
  If(-not (get-PackageProvider | ? {$_.Name -eq "NuGet" -and $_.Version -gt "2.8.5.208"})) {
  ...
  $modules = Get-SoftwareJSON
  ForEach($Param In $modules.Keys) {
    if($Software.$Param) {
  ...
  Install-Component -Component $Software.VSTS -Arguments "--unattended --url https://dev.azure.com/$TeamAccount --auth pat --token $PATToken --pool ""$PoolName"" --agent $ENV:COMPUTERNAME --replace --work C:\agent\_work --runAsService"
  sc.exe config "vstsagent.$TeamAccount.$PoolName.$ENV:COMPUTERNAME" obj="Localsystem"
  Restart-Service -Name "vstsagent.$TeamAccount.$PoolName.$ENV:COMPUTERNAME"
